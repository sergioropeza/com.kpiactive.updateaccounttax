		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (payment != null && invoice != null && isMXTaxCorrection) {
				...
				for (MInvoiceTax invoiceTax : invoiceTaxes) {
					...
					Integer T_TaxPaidPO_Acct = taxtAcct.get_ValueAsInt("T_TaxPaidPO_Acct");
					Integer PaidTaxCombination_ID = invoice.isSOTrx()?T_TaxPaidSO_Acct:T_TaxPaidPO_Acct;
					
					if (PaidTaxCombination_ID==null || PaidTaxCombination_ID ==0)
						continue;
					...
					BigDecimal percRate = rate.divide(Env.ONEHUNDRED, 2, RoundingMode.HALF_UP);
					BigDecimal base = amtDifference.divide(Env.ONE.add(percRate), 2, RoundingMode.HALF_UP);
					BigDecimal TaxDifference = amtDifference.subtract(base);
					PaidTaxAmt = PaidTaxAmt.subtract(amtRet);
					if (invoice.isSOTrx()) {
						fl = fact.createLine (line, MAccount.get(PaidTaxCombination_ID),getC_Currency_ID(), null, PaidTaxAmt);
						fl.setAD_Org_ID(invoice.getAD_Org_ID());
											
						fl = fact.createLine (line, accountNotPaid,getC_Currency_ID(), PaidTaxAmt, null );
						fl.setAD_Org_ID(invoice.getAD_Org_ID());
						BigDecimal AcctDr =  fl.getAmtAcctDr().add(TaxDifference);
						fl.setAmtAcctDr(AcctDr);
						fl.saveEx();
												
					}else {
						fl = fact.createLine (line, MAccount.get(PaidTaxCombination_ID),getC_Currency_ID(), PaidTaxAmt,null );
						fl.setAD_Org_ID(invoice.getAD_Org_ID());					
						
						fl = fact.createLine (line, accountNotPaid,getC_Currency_ID(), null, PaidTaxAmt );
						fl.setAD_Org_ID(invoice.getAD_Org_ID());
						BigDecimal AcctCr =  fl.getAmtAcctCr().add(TaxDifference);
						fl.setAmtAcctCr(AcctCr);
						fl.saveEx();
